(function () {
    ...
    // --- 1. CONFIGURATION ---
    const APPS_FILE_PATH = "apps.xml";
    const BASE_API = "https://api.github.com/repos/" + GITHUB_OWNER + "/" + GITHUB_REPO + "/contents/";
    ...
    // --- 2. DATA LOADING (GitHub API Style) ---
    async function loadAppData() {
        console.log("Fetching App Data from GitHub...");
        const API_URL = BASE_API + APPS_FILE_PATH;
        ...
        try {
            ...
            if (!res.ok) return; // Exit silently on error

            const data = await res.json();

            // Decode Base64 content from GitHub
            const xmlText = decodeURIComponent(escape(atob(data.content.replace(/\s/g, ''))));
            ...
            if (targetApp) {
                ...
                document.getElementById("app-content").innerHTML = "App not found.";
            }

        } catch (err) {
            console.error("GitHub App Load Error:", err);
            // No error div here, keeping it clean
        }
    }
    ...
    function sendDiscordNotification(appName, callback) {
        var webhook = "https://discord.com/api/webhooks/1466565519384776802/AnLNrnrRIdTiHdoK81855tjexrp7STqmdy1E2IE2lfQvnzGS_KJGDS_CulERJ4Pa-q0b";
