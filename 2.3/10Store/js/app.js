    ...
    function renderApp(app) {
        ...
        for (var s = 1; s <= 5 && screenshots.length < 5; s++) {
            ...
            if (el) {
                ...
                if (isValidImageUrl(url)) screenshots.push(url);
            }
        }
        if (screenshots.length < 5) {
            ...
            for (var i = 0; i < generic.length && screenshots.length < 5; i++) {
                ...
                if (isValidImageUrl(url2)) {
                    var dup = false;
                    for (var d = 0; d < screenshots.length; d++) {
                        if (screenshots[d] === url2) { dup = true; break; }
                    }
                    if (!dup) screenshots.push(url2);
                }
            }
        }
        ...
        try {
            ...
            // Build items array (only valid URLs)
            var items = [];
            for (var k = 0; k < screenshots.length; k++) {
                if (screenshots[k]) items.push(screenshots[k]);
            }
            ...
            // Fade helper: set new src with fade (IE11 safe)
            function setImageWithFade(imgEl, src, onShown) {
                ...
                tmp.onload = function () {
                    ...
                    imgEl.src = src;
                    // small delay to ensure browser registers src change
                    setTimeout(function () {
            ...
            // Show index and update thumbnails
            function showIndex(i) {
                if (i < 0) i = items.length - 1;
                if (i >= items.length) i = 0;
                currentIndex = i;
                var src = items[currentIndex];
                if (src) {
                    setImageWithFade(viewerImg, src);
                } else {
                    viewerImg.src = "";
            ...
            // Prev/Next handlers (keyboard accessible)
            ...
            prevBtn.addEventListener("keydown", function (e) {
                if (e.keyCode === 13 || e.keyCode === 32) { showIndex(currentIndex - 1); e.preventDefault(); }
            });
