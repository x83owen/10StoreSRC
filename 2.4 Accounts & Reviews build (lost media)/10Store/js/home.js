(function () {
    ...
    const API_URL = `https://api.github.com/repos/${GITHUB_OWNER}/${GITHUB_REPO}/contents/${APPS_FILE_PATH}`;
    ...
    function renderHero(container) {
        ...
        featuredApps.forEach(function (app, index) {
            ...
            slide.onclick = function () { window.location.href = 'app.html?id=' + app.getAttribute("id"); };
    ...
    // --- Main Loader (GitHub API Version) ---
    async function loadHomeContent() {
        console.log("Loading Home content from GitHub...");
        ...
        try {
            ...
            if (!res.ok) throw new Error("Failed to fetch apps.xml from GitHub");
    ...
    function fillGrid(grid, apps, filter) {
        ...
        for (var j = 0; j < apps.length; j++) {
            if (filter(apps[j]) && isCompatible(apps[j])) {
                ...
                setTimeout(function (c) { c.classList.add("visible"); }, 50, card);
                idx++;
            }
        }
    }
